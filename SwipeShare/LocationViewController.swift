...
//        delegate?.toggleSettingsPanel?()
    ...
    */
                ...
                do {
                    ...
                    if users.count != 0 {
                        ...
                        if sender.profImageData == nil {
                            // Fetch profile picture from parse and store in core data
                            getProfilePicture(messageSender, storedSender: sender)
                            
                        }
                        
                    } else {        // Create a new User entity to store
                        print("creating new user entity to store")
    ...
    func getProfilePicture(messageSender: PFUser, storedSender: User) {
                ...
                if let picture = object!["profilePicture"] as? PFFile {
                    ...
                    picture.getDataInBackgroundWithBlock { (imageData: NSData?, error: NSError?) -> Void in
                        if (error == nil) {
                            ...
                            storedSender.profImageData = imageData
